...
func VerifyTLS(host string, port int, wantSerial string, wantSANs []string) (model.VerificationResult, error) {
	return VerifyTLSContext(context.Background(), host, port, wantSerial, wantSANs)
}
func VerifyTLSContext(ctx context.Context, host string, port int, wantSerial string, wantSANs []string) (model.VerificationResult, error) {
	result := model.VerificationResult{CheckedAt: time.Now().UTC()}
	address := net.JoinHostPort(host, strconv.Itoa(port))
	dialer := &net.Dialer{Timeout: 8 * time.Second}
	raw, e := dialer.DialContext(ctx, "tcp", address)
	if e != nil { result.Error = e.Error(); return result, e }
	conn := tls.Client(raw, &tls.Config{ServerName: host, MinVersion: tls.VersionTLS12})
	if e = conn.HandshakeContext(ctx); e != nil { result.Error = e.Error(); _ = conn.Close(); return result, e }
	defer conn.Close()
