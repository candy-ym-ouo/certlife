...
func (s *StatsService) Dashboard(ctx context.Context) (Stats, error) {
	...
	out := Stats{Total: total, ByStatus: map[model.CertificateStatus]int{}, Renewals30D: map[string]int{"total": 0, "succeeded": 0, "failed": 0}, NotificationsToday: map[string]int{"sent": 0, "failed": 0}}
	for _, c := range items {
