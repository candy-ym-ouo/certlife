...
type DeployService struct {
	...
	dir      string
	rollback bool
	verified map[int64]model.VerificationResult
}

func NewDeployService(s *store.DeploymentStore, dir string, rollback bool) *DeployService {
	return &DeployService{targets: s, dir: dir, rollback: rollback}
}
...
func (s *DeployService) RememberVerification(id int64, result model.VerificationResult) {
	s.verified[id] = result
}
